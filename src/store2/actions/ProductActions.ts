  ...
  try {
    ...
    if(loaders.compare){
      if (!inCompare) {
          ...
          await addItemToCompare({compareId:user.compare,product:id})
...
export const addToLoves = (id:string | undefined)=>async (dispatch:AppDispatch) => {
  ...
  const {setCompare,setLoves} = navbarSlice.actions
  const element = document.querySelector('.Navbar__loader')
  try {
    if (!user.id) {
      ...
      if(loaders.love){
        ...
        await addProductInLoves({lovesId:user.loves,product:id})
        dispatch(setInLoves(true))
        dispatch(setLoves(navbar.loves + 1))
  ...
  try {
    ...
    if(loaders.basket){
      if (!inBasket) {    
          ...
          await addItemToBasket({basketId:user.basket,product:id,count:1})
          ...
          dispatch(setLoaders({...loaders,basket:false}) )
          await removeItemFromBasket({id:basket.find((el:any)=>el.product._id === id)?.product._id,basketId:user.basket})
