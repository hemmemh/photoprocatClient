...
export const getAllInfo = (typeId: any) => async (dispatch: AppDispatch) => {
    ...
    const { checkedBrands, limit, user } = currentState.reducer.catalog
    const { setBrands, setBrandsLoad, setCheckedBrands, setProducts, setProductsLoad, setBasket, setCompare, setLoves, setType, setPage } = catalogSlice.actions
    dispatch(setProductsLoad(false))
    dispatch(setPage(1))
    try {
        const data = await getAllproduct(typeId, 1, limit, null, null, null, null, null, null, null, null)
        dispatch(setProducts({ ...data }))
        dispatch(setType(data.responce[0].type.name))
        const { setTypeInformation, setInformations, setInformationValues, setsliderMouseOn, setPriceRange, setMinMaxPrice, setPriceValue } = catalogSlice.actions
        ...
        Object.entries(typeInformationConst).forEach((el: any) => {
            const type = el[1]
            const typeName = el[0]
            let arr: any = []
...
export const getProducts = (action: any[]) => async (dispatch: AppDispatch) => {
    const { setProducts, setProductsLoad, setPage } = catalogSlice.actions
    dispatch(setProductsLoad(false))
    try {
...
export const updateProducts = (action: any[]) => async (dispatch: AppDispatch) => {
    try {
        const currentState = store.getState()
        const { products, page } = currentState.reducer.catalog
...
export const addRaitingToProduct = (id: string | undefined, raitingModal: number | null, sername: string, text: string, name: string) => async (dispatch: AppDispatch) => {
    ...
    const { user } = currentState.reducer.catalog
    const { setRaiting, setProduct } = productSlice.actions
...
export const addProduct = (files: File[]) => async (dispatch: AppDispatch) => {
    const currentState = store.getState()
    const { setName, setDescription, setPrice, setBrand, setType } = adminSlice.actions
    const { brand, type, typeInformationProduct, name, description, price } = currentState.reducer.admin
    try {
        let bool = true
        ...
        if (name !== '' && description !== '' && price !== '0' && bool && '_id' in type && '_id' in brand && files.length > 1) {
